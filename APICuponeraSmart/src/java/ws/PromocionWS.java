            ...
            try {
                ...
                if (numFilasAfectadasCupon > 0) {
                    ...
                    if (numFilasAfectadasPromo > 0) {
                        // Obtener el último ID de la promoción
                        int ultimoIdPromocion = conexionBD.selectOne("promocion.obtenerUltimoIdPromocion");
                        promo.setIdPromocion(ultimoIdPromocion);
                        System.out.println("idPromo" + ultimoIdPromocion);

                        conexionBD.commit();
                        msj.setError(false);
                        msj.setMensaje("Información de la promoción y cupones registrada correctamente. "
                                + "Para agregar una o varias sucursales, favor de editarla");
                    } else {
                        conexionBD.rollback();
                        msj.setError(true);
